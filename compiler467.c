...
#if 1
    if(genFile!=NULL)fclose(genFile);
    genFile = fopen("frag.txt", "w");
    if(genFile==NULL){
    printf("Error: Can not open frag.txt for write!\n");
    return 0;
    }
    fprintf(genFile,"!!ARBfp1.0\n");
    reg_info_init();  //initialize register pool
    GenCode(ast);
    fprintf(genFile,"END");
    fclose (genFile);
    //;
